...
export default function CustomBankAvatar({
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // Convert Blob to File before passing to setImageCropped
      const croppedFile = new File([croppedBlob], `${Date.now()}-avatar.jpg`, {
        type: "image/jpeg",
        lastModified: Date.now(),
      });
      setImageCropped(croppedFile);
        ...
        >
          {finalImageUrl ? (
            ...
            <div aria-hidden="true">
              <FaPiggyBank className="size-4 opacity-60" />
            </div>
          )}
        </button>
        ...
        <DialogContent className="gap-0 p-0 sm:max-w-140 *:[button]:hidden">
          ...
          <DialogHeader className="contents space-y-0 text-left">
            <DialogTitle className="flex items-center justify-between border-b p-4 text-base">
              ...
              <Button
                className="bg-dark-background -my-1"
                onClick={handleApply}
                disabled={!previewUrl}
                autoFocus
              >
